import Prism, { Grammar, Token as PrismToken, TokenStream } from "prismjs";
...
export default function Code({
  ...
  const tokens = normalizeTokens(config.tokens);
  const highlights = parseHighlight(highlight);
  return (
    <div
      className={[
        styles.container,
        highlights.length ? styles.highlit : null,
        className,
      ]
        .filter(Boolean)
        .join(" ")}
    >
      {tokens.map((line, index) => (
        <div
          ...
          className={[
            styles.line,
            highlights.includes(index + 1) ? styles.highlight : "",
          ].join(" ")}
        >
